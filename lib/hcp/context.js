...
var IGNORED_FILES_CONFIG_PATH = path.join(process.cwd(), '.hcpignore');
var DEFAULT_WWW_FOLDER = path.join(process.cwd(), 'www');
var DEFAULT_CLI_CONFIG = path.join(process.cwd(), 'hcp-default.json');
...
var Context = function(argv) {
    ...
    this.manifestFilePath = path.join(this.sourceDirectory, 'hcp.manifest');
    this.projectsConfigFilePath = path.join(this.sourceDirectory, 'hcp.json');
