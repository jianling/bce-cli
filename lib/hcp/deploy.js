...
var credentialsFile = path.join(process.cwd(), '.chcplogin');
...
function getDeployInfo(context) {
    ...
    try {
        config = fs.readFileSync(context.projectsConfigFilePath, 'utf8');
...
function deploy(context, config) {
    ...
    files.pipe(es.mapSync(function (entry) {
        ...
        var fullPath = path.join(context.sourceDirectory, entry.path);

        bosClient.putObjectFromFile(bucket, key, fullPath)
            .then(function(response) {
                console.log('upload ' + fullPath + ' to ' + config.endpoint + '/' + bucket + '/' + key);
            })
