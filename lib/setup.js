...
Setup.startApp = function startApp(options) {

    console.log('Creating in ' + options.targetPath);

    return Setup.fetchBase(options)
    .then(function() {
        console.log('Installing npm packages...');
        return Setup.runSpawnCommand('npm', ['install']);
    })
    .then(function() {
        return Setup.addCordovaPlugins(options);
    })
    .then(function() {
        return Setup.addDefaultPlatforms(options);
    });
};

Setup.fetchBase = function(options) {
    var q = Q.defer();
...
Setup.runSpawnCommand = function(cmd, args) {
    var q = Q.defer();
...
Setup.addCordovaPlugins = function(options) {
    var promises = [];
...
Setup.addDefaultPlatforms = function(options) {
    var promises = [];
...
Setup.buildCommonSass = function(options) {
    return sassBuilder.render(options);
};
